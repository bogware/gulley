...
**Delivered** (M6.1–M6.3, `terraform validate` + YAML-parse clean). **M6.1** six Terraform modules + a composing `stack` + `dev`(single-NAT)/`prod`(multi-AZ, HA, deletion-protection) roots: `network` (VPC, subnets, NAT, tiered SGs, VPC interface endpoints for ecr/secretsmanager/kms/logs/sts/bedrock-runtime + S3 gateway endpoint), `security` (split-KMS per class + rotation, empty Secrets Manager entries, least-privilege IAM task roles, optional cross-account Bedrock assume-role with ExternalId), `data` (Aurora Serverless v2 with RDS-managed password, Redis×3 role-split eviction, S3 Object Lock COMPLIANCE bucket), `compute` (ECS cluster, **SSE-tuned ALB** idle 300s + dereg 180s + TLS1.3, ARM64 Fargate task defs, circuit-breaker deploys, request-count autoscaling), `edge` (DNS-validated ACM), `observability` (log group + immutable scan-on-push ECR). Real Terraform v1.15.8: fmt + validate SUCCESS for both roots. **M6.2** bounded graceful SIGTERM drain in both mains (backstop under the 120s stopTimeout, upstream-pool close, Last-Event-ID reconnect contract) + Node-as-PID-1 Dockerfiles + `initProcessEnabled`. **M6.3** CI parity via shared `ci/tf-check.sh` + `ci/build-image.sh` (ARM64 → ECR with SBOM + provenance); GitHub Actions `ci` (verify + terraform), `release` (OIDC, build/push, Trivy HIGH/CRITICAL gate, prod plan), and `dependency-bump` (**workflow_dispatch only — no schedule/Dependabot**); Azure DevOps at parity. The M1–M5 credential-leak / ReDoS / guard fuzz tests run in `verify`, so the security review is a standing CI gate. **Seam:** `terraform apply` + the CI image build run against a real account/agent (validated but not applied this session; no Docker daemon).
